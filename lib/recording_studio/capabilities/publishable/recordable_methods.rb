...
module RecordingStudio
  module Capabilities
    module Publishable
      module RecordableMethods
        extend ActiveSupport::Concern

        def self.configure!(base, **options)
          path = options.fetch(:path, RecordingStudioPublishable::Configuration::DEFAULT_PUBLIC_PATH)
          configured_path = RecordingStudioPublishable.configuration.public_path_for(base.name)
          effective_path = path
          ...
          RecordingStudioPublishable.configuration.register_public_renderer(
            base.name,
            controller: options[:public_controller],
            action: options[:public_action],
            layout: options[:public_layout],
            path: effective_path
          )
        end
