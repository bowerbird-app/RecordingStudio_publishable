...
module RecordingStudioPublishable
  module RendersPublicPage
    extend ActiveSupport::Concern

    private

    def render_public_publishable_page(publishable_recording, preview: false)
      assign_public_publishable(publishable_recording)
      return head :not_found unless public_page_allowed?(preview)
      return if redirect_stale_public_slug?(preview)

      @publishable_preview = preview
    ...
    def assign_public_publishable(publishable_recording)
      @publishable_recording = publishable_recording
      @publishable = publishable_recording&.recordable
    end

    def public_page_allowed?(preview)
      @publishable.present? && (preview || @publishable.currently_published?)
    end

    def redirect_stale_public_slug?(preview)
      return false if preview

      canonical_path = public_canonical_path_for(@publishable_recording)
      return false unless stale_slug? && canonical_path.present?

      redirect_to_canonical_path(canonical_path)
      true
    end
