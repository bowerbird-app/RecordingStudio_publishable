...
Canonical URL is an optional override on the SEO screen, under Advanced. Leave it blank to use the public URL. Fill it only when this page is replacing an older address. Keep this out of search engines is on that same Advanced panel. It writes `noindex` so the page stays live while search engines skip it. It is not a site-index hide. The SEO screen and the Update service both accept those fields, including when SEO tags are turned off for that type.
