...
class HooksTest < Minitest::Test
  def setup
    @hooks = RecordingStudioPublishable::Hooks.new
  end
  ...
  def test_raise_on_error_raises_hook_error
    ...
    assert_raises(RecordingStudioPublishable::Hooks::HookError) do
      @hooks.run(:after_initialize)
    end
  end
  ...
  def test_class_run_delegates_to_configuration
    RecordingStudioPublishable.configuration.hooks
    called = false

    RecordingStudioPublishable.configuration.hooks.after_initialize { called = true }
    RecordingStudioPublishable::Hooks.run(:after_initialize)

    assert called
  ensure
    RecordingStudioPublishable.configuration.hooks.clear!
  end

  def test_class_trigger_is_alias_for_run
    called = false
    RecordingStudioPublishable.configuration.hooks.on(:custom_event) { called = true }

    RecordingStudioPublishable::Hooks.trigger(:custom_event)
  ...
  def test_class_run_around_delegates_to_configuration
    events = []
    RecordingStudioPublishable.configuration.hooks.around_service do |_context, block|
      events << :around
      block.call
    end

    result = RecordingStudioPublishable::Hooks.run_around(:around_service, :service) do
      events << :core
      :ok
    end
